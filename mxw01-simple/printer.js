// MXW01 Thermal Printer Protocol (JavaScript version)
// Converted from TypeScript for standalone use

const PRINTER_WIDTH = 384;
const PRINTER_WIDTH_BYTES = PRINTER_WIDTH / 8; // 48 bytes
const MIN_DATA_BYTES = 90 * PRINTER_WIDTH_BYTES; // 4320 bytes minimum
...
class MXW01Printer {
  constructor(controlWrite, dataWrite) {
  ...
  async setIntensity(intensity = 0x5d) {
  ...
  async printRequest(lines, mode = 0) {
  ...
  async sendDataChunks(data, chunkSize = PRINTER_WIDTH_BYTES) {
  ...
  async waitForPrintComplete(timeoutMs = 20000) {
    this.printComplete = false;
